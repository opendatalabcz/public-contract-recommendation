...
class STSEval(object):
    ...
    def do_prepare(self, params, prepare):
        ...
        return prepare.run(params, self.samples)
...
class STS162Eval(STSEval):
    def __init__(self, taskpath, seed=1111):
        logging.debug('***** Transfer task : STS216 *****\n\n')
        self.seed = seed
        self.datasets = ['answer-answer', 'headlines', 'plagiarism',
                         'postediting', 'question-question']
        self.loadFile(taskpath)
    ...
    def loadFile(self, fpath):
        ...
        for dataset in self.datasets:
            ...
            gs_scores = [float(x) for x in raw_scores[not_empty_idx]]
            sent1 = np.array([hashlib.md5(s.encode('utf-8')).hexdigest() for s in sent1])[not_empty_idx].tolist()
            sent2 = np.array([hashlib.md5(s.encode('utf-8')).hexdigest() for s in sent2])[not_empty_idx].tolist()

            self.data[dataset] = (sent1, sent2, gs_scores)
            self.samples += sent1 + sent2

class STSCZEval(STSEval):
    def __init__(self, taskpath, seed=1111):
        logging.debug('***** Transfer task : STSCZ *****\n\n')
        self.seed = seed
        self.datasets = ['headlines', 'headlines2013-2015CZ_Lemma', 'headlines2013-2015CZ_POSTag', 'headlines2013-2015CZ_Stem',
                         'images', 'imagesCZ2014-2015shuffled', 'imagesCZ2014-2015shuffled_Lemma', 'imagesCZ2014-2015shuffled_POSTag', 'imagesCZ2014-2015shuffled_Stem']
        self.loadFile(taskpath)

class STSCZ2Eval(STSEval):
    def __init__(self, taskpath, seed=1111):
        logging.debug('***** Transfer task : STSCZ2Eval *****\n\n')
        self.seed = seed
        self.datasets = ['headlines', 'headlines2013-2015CZ_Lemma', 'headlines2013-2015CZ_POSTag', 'headlines2013-2015CZ_Stem',
                         'images', 'imagesCZ2014-2015shuffled', 'imagesCZ2014-2015shuffled_Lemma', 'imagesCZ2014-2015shuffled_POSTag', 'imagesCZ2014-2015shuffled_Stem']
        self.loadFile(taskpath)
...
class STSCZsmallEval(STSEval):
    def __init__(self, taskpath, seed=1111):
        logging.debug('***** Transfer task : STSCZsmall *****\n\n')
        self.seed = seed
        self.datasets = ['headlines', 'images']
        self.loadFile(taskpath)
