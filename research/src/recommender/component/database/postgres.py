...
class ContractLocalityManager(DBManager):
    ...
    def __init__(self, connection, **kwargs):
        super().__init__(connection, **kwargs)
        self._load_query = """select c.contract_id, e.address, e.latitude, e.longitude
...
class ContractEntitySubjectManager(DBManager):
    ...
    def __init__(self, connection, **kwargs):
        super().__init__(connection, **kwargs)
        self._load_query = """select c.contract_id, es.description, es.feature
                              from contract c join
                                submitter s on c.submitter_id=s.submitter_id join
...
class UserProfileManager(DBManager):
    ...
    def __init__(self, connection, **kwargs):
        super().__init__(connection, **kwargs)
        self._load_query = """select u.user_id, u.address, u.latitude, u.longitude, i.item_desc, i.feature
